  ...
  function loadStitchedSDOVideo() {
    ...
    const videoHTML = `
      <video autoplay muted loop playsinline>
        <source src="https://www.fitsflow.org/daily_videos/${dateStr}_stitched.mp4" type="video/mp4">
...
function getBaseUrl(id) {
  return `https://www.fitsflow.org/temp/${id}/processed`;
}
  ...
  <div style="display: flex; gap: 40px; border-top: 2px solid #555; padding-top: 30px;">
    ...
    <div style="flex: 1; text-align: center;">
      ...
      <p style="margin-top: 25px;">
        <a href="https://www.fitsflow.org/examples/05-01-2013-304/0304.zip"
    ...
    <div style="flex: 1; border-left: 1px solid #444; padding-left: 30px; text-align: center;">
      ...
      <p style="margin-top: 25px;">
        <a href="https://www.fitsflow.org/examples/collection/collection.zip"
           style="color: #5ccaff; text-decoration: underline; font-size: 16px;" target="_blank">
...
function generateUniqueID(length = 20) {
  ...
  async function pollLogs(session_id, filenames) {
    const baseUrl = `https://www.fitsflow.org/temp/${session_id}/logs`;
  ...
  submitButton.addEventListener('click', async () => {
    ...
    const resetBtn = document.getElementById('resetFitsUpload');
    ...
    if (sound) {
      sound.play().then(() => {
        sound.pause();
        sound.currentTime = 0;
      }).catch(e => console.log("Sound priming failed:", e));
    }
    ...
    if (files.length === 0) {
      ...
      resetBtn.disabled = false;              // ✅ re-enable reset on early return
      return;
    }
  
    const session_id = generateUniqueID();    // ✅ declare locally
    //console.log("📦 Session ID:", session_id);
  
    // ✅ presign helper (explicit .toString())
    async function getPutUrl({ session_id, file }) {
      const qs = new URLSearchParams({
        session_id,
        filename: file.name,
        contentType: file.type || "application/fits"
      }).toString();
  
      //const res = await fetch(`https://gu3rh7bvn7.execute-api.us-east-1.amazonaws.com/default/FitsFlow-2-Trigger?${qs}`);
      const res = await fetch(`https://api.fitsflow.org/FitsFlow-2-Trigger?${qs}`);
      if (!res.ok) throw new Error(`presign failed: ${res.status}`);
      const { url } = await res.json();   // { url, key? }
      return url;
    }
  
    // ✅ upload loop (keep PUT); ensure SSE header matches how you presigned
    const uploadPromises = files.map(async (file) => {
      const url = await getPutUrl({ session_id, file });
      const putRes = await fetch(url, {
        method: "PUT",
        headers: {
          "Content-Type": file.type || "application/fits",
          "x-amz-server-side-encryption": "AES256"  // <-- keep only if you signed this
        },
        body: file
      });
      if (!putRes.ok) throw new Error(`${file.name} failed (HTTP ${putRes.status})`);
      //console.log(`✅ Uploaded: ${file.name} (ETag: ${putRes.headers.get('ETag') || 'n/a'})`);
    });
    ...
    try {
      ...
      fetch(`https://api.fitsflow.org/FitsFlow-2-Trigger?session_id=${encodeURIComponent(session_id)}`)
        .then(res => res.json())
        .then(response => {
          ...
          const baseNames = filenames.map(f => f.replace(/\.fits$/i, "")); // ✅ case-insensitive
          //console.log(baseNames);

          const baseUrl = getBaseUrl(session_id);

  
          playlist = baseNames.map(baseName => ({
            data_csv: `${baseUrl}/data/${baseName}_data.csv`,
            data_npy: `${baseUrl}/data/${baseName}_data.npy`,
            header:   `${baseUrl}/header/${baseName}_header.txt`,
            hek:      `${baseUrl}/hek/${baseName}_hek.json`,
            asdf:     `${baseUrl}/asdf/${baseName}.asdf`,
            sdo:      `${baseUrl}/images/sdo/${baseName}_sdo.png`,
            jsoc:     `${baseUrl}/images/jsoc/${baseName}_jsoc.png`,
          }));
          ...
          pollLogs(session_id, baseNames).then(() => {
            ...
            if (playlist.length <= 1) controls.forEach(btn => btn.disabled = true);
            else controls.forEach(btn => btn.disabled = false);
  
            if (playlist.length === 0) {
              handleAllFilesSkipped();
              // fall through to re-enable + hide overlay
            }
  
            // ✅ UI cleanup after processing completes
            submitButton.disabled = false;
            resetBtn.disabled = false;
            if (typeof hidePollingOverlay === 'function') hidePollingOverlay();
          });
        })
        .catch(err => {
          ...
          submitButton.disabled = false;     // ✅ also re-enable reset on error
          resetBtn.disabled = false;
          if (typeof hidePollingOverlay === 'function') hidePollingOverlay();
        });
      ...
      logToTerminal(`Something went wrong: ${err.message || err}`);
      submitButton.disabled = false;         // ✅ re-enable both on upload error
      resetBtn.disabled = false;
      if (typeof hidePollingOverlay === 'function') hidePollingOverlay();
    }
  });
...
function generateReadmeFromPlaylist(playlist) {
  ...
  ⚠️ Access expires at midnight Eastern Standard Time (EST). Be sure to download and save any needed files before then.
